...
public class dayanandEmployeeController {
    ...
    private static final Logger logger = LoggerFactory.getLogger(dayanandEmployeeController.class);

    @Autowired
    private EmployeeService EmployeeService;

    @GetMapping
    public ResponseEntity<List<dayanandEmployee>> getAllEmployees() {
        ...
        List<dayanandEmployee> Employees = EmployeeService.getAllEmployees();
        return ResponseEntity.ok(Employees);
    }

    @GetMapping("/{id}")
    public ResponseEntity<dayanandEmployee> getEmployeeById(@PathVariable("id") int id) {
        logger.info("FINISHED PROCESSING : METHOD=GET; REQUESTURI=/employees/{}; REQUEST PAYLOAD={}; RESPONSE CODE={}; RESPONSE={}; TIME TAKEN={}",
                id, HttpStatus.OK.value(), "Employee details", "N/A");
        dayanandEmployee Employee = EmployeeService.getEmployeeById(id);
        return ResponseEntity.ok(Employee);
    }

    @PostMapping
    public ResponseEntity<dayanandEmployee> addEmployee(@RequestBody dayanandEmployee Employee) {
        ...
        dayanandEmployee savedEmployee = EmployeeService.addEmployee(Employee);
